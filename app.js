(function () {
  ...
  function formatMetric(metric, value) {
    ...
    return metric === "payroll_annual_thousands" ? fmtPayroll(value) : fmtNumber(value);
  }
    ...
    const boards = Object.entries(categories)
      .map(([code, cat]) => {
        const rows = [
          ...
          .join("");
        return `<div class="board"><h3>${cat.label}</h3><dl>${rows}</dl></div>`;
      })
      .join("");
  ...
  function renderTable() {
    ...
    els.tableBody.innerHTML = rows || `<tr><td colspan="6">No states match “${els.search.value}”.</td></tr>`;
  }
